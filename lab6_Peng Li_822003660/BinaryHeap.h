...
public:
    ...
    int insertItem(const ElemType& x,Locator<L>& loc);
    int removeKey();
    void walkUp(int hole, int& count);
    void walkDown(int hole, int& count);
    int decreaseKey( Locator<L>& loc , int k);
    void printPQHeap();
};
...
//insertItem function
template<typename ElemType, typename Comp, typename L>
int BinaryHeap<ElemType, Comp, L>::insertItem(const ElemType& x,Locator<L>& loc) {
    int cnt = 0;
    if (curSize == 0) {
        array[0] = x;
        curSize++;
        ...
        curSize++;
        int hole = curSize-1;
        for ( ; hole > 0 && (cnt++,comp(array[(hole-1)/2], x))> 0; hole = (hole-1)/2){
...
//removeKey()
template<typename ElemType, typename Comp, typename L>
int BinaryHeap<ElemType, Comp, L>::removeKey()  {
    int cnt = 0;
...
// decreaseKey
template<typename ElemType, typename Comp, typename L>
int BinaryHeap<ElemType, Comp, L>::decreaseKey(Locator<L>& loc , int key){
    int cnt = 0;
